...
const protectedPages = [
  ...
  { path: "/employees/calendar", name: "Employee Calendar", heading: /calendar/i },
  ...
  { path: "/reports/13th-month", name: "13th Month Report", heading: /13th month/i },
  { path: "/reports/payroll-summary", name: "Payroll Summary", heading: /payroll summary/i },
  { path: "/reports/employees", name: "Employee Report", heading: /employee/i },
  { path: "/reports/earnings-deductions", name: "Earnings & Deductions", heading: /earnings|deductions/i },
  { path: "/reports/attendance", name: "Attendance Report", heading: /attendance/i },
  { path: "/reports/benefits-utilization", name: "Benefits Utilization", heading: /benefits/i },
  ...
  { path: "/system/restrictions", name: "Restrictions", heading: /restrictions/i },
...
test.describe("Smoke tests — all protected pages", () => {
  for (const page of protectedPages) {
    test(`${page.name} (${page.path}) renders without crashing`, async ({ browser }) => {
      const context = await browser.newContext();
      const p = await context.newPage();
      await gotoWithAuth(p, page.path);

      // Page should not show an error boundary
      await expect(p.locator("text=Something went wrong")).not.toBeVisible({ timeout: 5000 });
