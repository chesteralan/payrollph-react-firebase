...
describe("Company Flow — Switch → Verify Data Isolation", () => {
  ...
  describe("Employee data isolation between companies", () => {
    beforeEach(() => {
      addMockDocs("employees", [
        { id: "emp-a1", nameId: "name-a1", companyId: "company-a", employeeCode: "ACA001", isActive: true },
        { id: "emp-a2", nameId: "name-a2", companyId: "company-a", employeeCode: "ACA002", isActive: true },
        { id: "emp-b1", nameId: "name-b1", companyId: "company-b", employeeCode: "BIN001", isActive: true },
        { id: "emp-b2", nameId: "name-b2", companyId: "company-b", employeeCode: "BIN002", isActive: true },
        { id: "emp-b3", nameId: "name-b3", companyId: "company-b", employeeCode: "BIN003", isActive: false },
      ]);
    });
    ...
    it("should enforce data isolation by filtering on companyId", async () => {
      ...
      const companyAFilter = { field: "companyId" as const, op: "==" as const, value: "company-a" };
      const companyBFilter = { field: "companyId" as const, op: "==" as const, value: "company-b" };
      ...
      const whereCalls = vi.mocked(where).mock.calls.filter(
    ...
    it("should filter results in application layer for data isolation verification", async () => {
      ...
      const companyAEmps = allEmployees.filter((e) => e.companyId === "company-a");
      const companyBEmps = allEmployees.filter((e) => e.companyId === "company-b");
  ...
  describe("Payroll data isolation between companies", () => {
    beforeEach(() => {
      addMockDocs("payroll", [
        { id: "pr-a1", companyId: "company-a", name: "Jan 2025 A", month: 1, year: 2025, status: "locked" },
        { id: "pr-a2", companyId: "company-a", name: "Feb 2025 A", month: 2, year: 2025, status: "draft" },
        { id: "pr-b1", companyId: "company-b", name: "Jan 2025 B", month: 1, year: 2025, status: "locked" },
      ]);
    });
  ...
  describe("Template data isolation between companies", () => {
    beforeEach(() => {
      addMockDocs("payroll_templates", [
        { id: "tpl-a1", companyId: "company-a", name: "Standard Monthly A", isActive: true },
        { id: "tpl-a2", companyId: "company-a", name: "Semi-Monthly A", isActive: true },
        { id: "tpl-b1", companyId: "company-b", name: "Standard Monthly B", isActive: true },
      ]);
    });
    ...
    it("should isolate templates by company in application layer", async () => {
      ...
      const templatesA = all.filter((t: Record<string, unknown>) => t.companyId === "company-a");
      const templatesB = all.filter((t: Record<string, unknown>) => t.companyId === "company-b");
  ...
  describe("Employee group isolation between companies", () => {
    beforeEach(() => {
      addMockDocs("employee_groups", [
        { id: "grp-a1", name: "Engineering A", companyId: "company-a", isActive: true },
        { id: "grp-a2", name: "Sales A", companyId: "company-a", isActive: true },
        { id: "grp-b1", name: "Engineering B", companyId: "company-b", isActive: true },
      ]);
    });
    ...
    it("should isolate groups by company in application layer", async () => {
      ...
      const groupsA = all.filter((g: Record<string, unknown>) => g.companyId === "company-a");
      const groupsB = all.filter((g: Record<string, unknown>) => g.companyId === "company-b");
  ...
  describe("Full company switch lifecycle", () => {
    it("should perform complete switch and verify data isolation across entity types", async () => {
      addMockDocs("employees", [
        { id: "emp-a1", nameId: "n1", companyId: "company-a", employeeCode: "ACA001", isActive: true },
        { id: "emp-b1", nameId: "n2", companyId: "company-b", employeeCode: "BIN001", isActive: true },
      ]);
      addMockDocs("payroll", [
        { id: "pr-a1", companyId: "company-a", name: "Jan 2025", month: 1, year: 2025, status: "draft" },
        { id: "pr-b1", companyId: "company-b", name: "Jan 2025", month: 1, year: 2025, status: "locked" },
      ]);
      addMockDocs("payroll_templates", [
        { id: "tpl-a1", companyId: "company-a", name: "Monthly A", isActive: true },
        { id: "tpl-b1", companyId: "company-b", name: "Monthly B", isActive: true },
      ]);
      addMockDocs("employee_groups", [
        { id: "grp-a1", companyId: "company-a", name: "Engineering", isActive: true },
        { id: "grp-b1", companyId: "company-b", name: "Engineering", isActive: true },
      ]);

      // Verify query construction for Company A
      await getAll("employees", [{ field: "companyId", op: "==", value: "company-a" }]);
      expect(where).toHaveBeenCalledWith("companyId", "==", "company-a");

      // Verify query construction for Company B
      await getAll("payroll", [{ field: "companyId", op: "==", value: "company-b" }]);
      expect(where).toHaveBeenCalledWith("companyId", "==", "company-b");
      ...
      expect(allEmps.filter((e) => e.companyId === "company-a")).toHaveLength(1);
      expect(allEmps.filter((e) => e.companyId === "company-b")).toHaveLength(1);
      expect(allPrs.filter((p) => p.companyId === "company-a")).toHaveLength(1);
      expect(allPrs.filter((p) => p.companyId === "company-b")).toHaveLength(1);
      expect(allTpls.filter((t: Record<string, unknown>) => t.companyId === "company-a")).toHaveLength(1);
      expect(allTpls.filter((t: Record<string, unknown>) => t.companyId === "company-b")).toHaveLength(1);
      expect(allGrps.filter((g: Record<string, unknown>) => g.companyId === "company-a")).toHaveLength(1);
      expect(allGrps.filter((g: Record<string, unknown>) => g.companyId === "company-b")).toHaveLength(1);
    });

    it("should return different company-specific options via query construction", async () => {
      addMockDocs("company_options", [
        { id: "opts-a", companyId: "company-a", columnGroup: { dtr: true, salaries: true, earnings: true, benefits: true, deductions: true }, workDays: [1, 2, 3, 4, 5] },
        { id: "opts-b", companyId: "company-b", columnGroup: { dtr: false, salaries: true, earnings: true, benefits: false, deductions: true }, workDays: [1, 2, 3, 4, 5, 6] },
      ]);

      // Verify the service constructs correct filtered queries
      await getAll("company_options", [{ field: "companyId", op: "==", value: "company-a" }]);
      expect(where).toHaveBeenCalledWith("companyId", "==", "company-a");

      // Application-level filtering for data isolation
      const allOpts = await getAll("company_options");
      const optsA = allOpts.filter((o: Record<string, unknown>) => o.companyId === "company-a");
      const optsB = allOpts.filter((o: Record<string, unknown>) => o.companyId === "company-b");

      expect((optsA[0] as Record<string, unknown>).columnGroup as Record<string, boolean>).toHaveProperty("dtr", true);
      expect((optsB[0] as Record<string, unknown>).columnGroup as Record<string, boolean>).toHaveProperty("dtr", false);
    });
  });
});
