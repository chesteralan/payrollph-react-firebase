...
import { Plus, Trash2, Lock, Unlock, ArrowRight, Search, Eye, Copy } from 'lucide-react'
...
export function PayrollRunsPage() {
  ...
  const handlePublish = async (payroll: Payroll) => {
    if (!confirm(`Publish ${payroll.name}? This action cannot be undone.`)) return
    await updateDoc(doc(db, 'payroll', payroll.id), { status: 'published', isLocked: true })
    addToast({ type: 'success', title: 'Payroll published', message: `${payroll.name} has been published` })
    fetchPayrolls()
  }
  ...
  const handleClone = async (payroll: Payroll) => {
    ...
    const monthName = new Date(0, nextMonth - 1).toLocaleString('default', { month: 'long' })
    ...
    addToast({ type: 'success', title: 'Payroll cloned', message: `Created ${newPayroll.name}` })
    fetchPayrolls()
  }
  ...
  const filteredPayrolls = useMemo(() => {
    return payrolls.filter(p => {
      ...
      else if (statusFilter === 'locked') matchesStatus = p.isLocked && p.status !== 'published'
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? <tr><td colSpan={4} className="px-6 py-4 text-center text-gray-500">Loading...</td></tr>
                : paginatedPayrolls.length === 0 ? <tr><td colSpan={4} className="px-6 py-4 text-center text-gray-500">No payroll runs found</td></tr>
                  ...
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {p.status !== 'published' && (
                          <>
                            <Button variant="ghost" size="sm" onClick={() => handleClone(p)} title="Clone payroll">
                            ...
                            {p.isLocked && (
                              <Button variant="ghost" size="sm" onClick={() => handlePublish(p)} title="Publish payroll">
