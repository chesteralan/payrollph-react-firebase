...
export function PrintFormatsPage() {
  ...
  const [basicForm, setBasicForm] = useState({
    name: "",
    description: "",
    outputType: "register" as PrintFormat["outputType"],
  });
  const [layoutForm, setLayoutForm] = useState({
    paperSize: "A4" as PrintFormat["paperSize"],
    orientation: "portrait" as PrintFormat["orientation"],
    fontSize: "sm" as PrintFormat["fontSize"],
  });
                    ...
                    onChange={(e) =>
                      setLayoutForm({
                        ...
                        orientation: e.target
                          .value as PrintFormat["orientation"],
                      })
                    }
                  >
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <ToggleField
                    ...
                    onChange={(v) =>
                      setHeaderForm({ ...headerForm, showHeader: v })
                    }
                  />
                  <ToggleField
                    label="Show Footer"
                    ...
                    onChange={(v) =>
                      setHeaderForm({ ...headerForm, showFooter: v })
                    }
                  />
                  <ToggleField
                    label="Company Logo"
                  ...
                  <ToggleField
                    ...
                    onChange={(v) =>
                      setHeaderForm({ ...headerForm, showTitle: v })
                    }
                  />
                  <ToggleField
                    ...
                    onChange={(v) =>
                      setHeaderForm({ ...headerForm, showPeriod: v })
                    }
                  />
                  <ToggleField
            ...
            {wizardStep === 3 && (
              <div className="space-y-4">
                <p className="text-sm text-gray-600">
                  Select and order columns for register output. Drag not
                  supported - order is saved as listed.
                </p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {AVAILABLE_COLUMNS.map((col) => (
                    <button
                      key={col.id}
                      onClick={() =>
                        toggleItem(selectedColumns, col.id, setSelectedColumns)
                      }
                      className={`flex items-center gap-2 p-3 border rounded-lg text-sm transition-colors ${selectedColumns.includes(col.id) ? "border-primary-500 bg-primary-50 text-primary-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}
                    >
                ...
                <div className="p-4 bg-gray-50 rounded-lg space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <span className="capitalize">
                      {
                        OUTPUT_TYPES.find(
                          (t) => t.value === basicForm.outputType,
                        )?.label
                      }
                    </span>
