...
import { Activity, CheckCircle, XCircle, AlertTriangle, RefreshCw, Database, HardDrive, Users, Server, Clock } from 'lucide-react'
...
const MAJOR_COLLECTIONS = [
  ...
  'earnings', 'deductions', 'benefits', 'payroll', 'payroll_templates',
  'payroll_inclusive_dates', 'payroll_groups', 'payroll_employees', 'salaries',
  'dtr_entries', 'companies', 'user_accounts', 'system_audit', 'backups'
]

const COMMON_QUERIES = [
  { name: 'Payroll by date range', query: 'payroll (orderBy date, where companyId)', needsComposite: true },
  { name: 'Employees by company', query: 'employees (where companyId, orderBy lastName)', needsComposite: true },
  { name: 'Audit log by timestamp', query: 'system_audit (orderBy timestamp desc)', needsComposite: false },
  { name: 'Payroll by status', query: 'payroll (where status, orderBy date)', needsComposite: true },
...
export function HealthCheckPage() {
  ...
  const [storageHealth, setStorageHealth] = useState<{ accessible: boolean; fileCount: number; totalSize: number; error?: string }>({ accessible: false, fileCount: 0, totalSize: 0 })
  const [authHealth, setAuthHealth] = useState<{ userCount: number; error?: string }>({ userCount: 0 })
  const [overallScore, setOverallScore] = useState<{ status: 'good' | 'warning' | 'error'; passCount: number; failCount: number; warningCount: number }>({ status: 'good', passCount: 0, failCount: 0, warningCount: 0 })
  ...
  const checkOnlineStatus = (): HealthCheckResult => {
    ...
    return {
      ...
      details: online ? 'Browser reports network connectivity' : 'Browser reports no network connectivity'
    }
  }

  const checkCollections = async (): Promise<{ results: HealthCheckResult[]; health: CollectionHealth[] }> => {
  ...
  const checkIndexes = (): HealthCheckResult[] => {
    return COMMON_QUERIES.map(q => ({
      ...
      message: q.needsComposite ? 'May require composite index' : 'No composite index needed',
      details: q.query
    }))
  }
  ...
  const checkAuth = async (): Promise<{ result: HealthCheckResult; health: typeof authHealth }> => {
    try {
  ...
  const runHealthCheck = useCallback(async () => {
    ...
    const { results: collectionResults, health: collHealth } = await checkCollections()
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-900">{overallScore.passCount + overallScore.failCount + overallScore.warningCount}</div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-2xl font-bold text-green-600">{overallScore.passCount}</div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-2xl font-bold text-yellow-600">{overallScore.warningCount}</div>
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <span className={`text-sm font-medium ${
              overallScore.status === 'good' ? 'text-green-600' :
              overallScore.status === 'warning' ? 'text-yellow-600' : 'text-red-600'
            }`}>
              {overallScore.status === 'good' ? 'Good' :
               overallScore.status === 'warning' ? 'Needs Attention' : 'Critical'}
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {results.map((result, index) => (
              <div key={index} className="flex items-start gap-3 p-3 border border-gray-100 rounded-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Database className="w-5 h-5" />Collection Health
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {collectionHealth.map((col) => (
                <div key={col.name} className="flex items-center justify-between p-2 border border-gray-100 rounded">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <HardDrive className="w-5 h-5" />Storage & Auth Health
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                <StatusIcon status={storageHealth.accessible ? 'pass' : 'fail'} />
              ...
              {storageHealth.error && (
                <p className="text-xs text-red-500 mt-1">{storageHealth.error}</p>
              )}
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">{authHealth.userCount} registered user{authHealth.userCount !== 1 ? 's' : ''}</span>
              </div>
              {authHealth.error && (
