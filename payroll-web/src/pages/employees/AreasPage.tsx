...
export function AreasPage() {
  ...
  const handleToggleStatus = async (area: EmployeeArea) => {
    await updateDoc(doc(db, 'employee_areas', area.id), { isActive: !area.isActive })
    fetchAreas()
  }
  ...
  const filteredAreas = useMemo(() => {
    ...
    return areas.filter(a => a.name.toLowerCase().includes(q) || (a.description || '').toLowerCase().includes(q))
  }, [areas, searchQuery])

  const { items: sortedAreas, handleSort, sortConfig } = useTableSort(filteredAreas, 'name')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canAdd('employees', 'areas') && (<Button onClick={() => setShowForm(!showForm)}><Plus className="w-4 h-4 mr-2" />Add Area</Button>)}
      </div>
      {showForm && (
        <Card>
      ...
      <Card>
        <CardContent className="pt-4">
          <div className="relative max-w-sm">
            ...
            <input type="text" placeholder="Search areas..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
        </CardContent>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? <tr><td colSpan={4} className="px-6 py-4 text-center text-gray-500">Loading...</td></tr>
                : sortedAreas.length === 0 ? <tr><td colSpan={4} className="px-6 py-4 text-center text-gray-500">No areas found</td></tr>
                : sortedAreas.map((a) => (
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <button onClick={() => handleToggleStatus(a)} className={`inline-flex px-2 py-1 text-xs font-medium rounded-full transition-colors ${a.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
