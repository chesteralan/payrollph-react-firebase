...
export function DeductionsPage() {
  ...
  const fetchItems = async () => {
    ...
    setItems(
      snap.docs.map((d) => ({ id: d.id, ...d.data() })) as DeductionItem[],
    );
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...
      type: formData.type,
      ruleValue:
        formData.type === "percentage" ? formData.ruleValue : undefined,
      isActive: true,
    };
    if (editingId) {
      await updateDoc(doc(db, "deductions", editingId), data);
    } else {
      await addDoc(collection(db, "deductions"), data);
    }
    setShowForm(false);
    setEditingId(null);
    setFormData({ name: "", description: "", type: "fixed", ruleValue: 0 });
    fetchItems();
  };
  ...
  const handleToggleStatus = async (item: DeductionItem) => {
    await updateDoc(doc(db, "deductions", item.id), {
      isActive: !item.isActive,
    });
    fetchItems();
  };
  ...
  const handleExportXLS = () => {
    ...
    exportToXLS(data, {
      filename: "Deductions",
      columns: deductionExportColumns,
