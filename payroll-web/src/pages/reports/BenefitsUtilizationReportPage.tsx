...
import type { Payroll, PayrollEmployeeBenefit, BenefitItem, PayrollEmployee, EmployeeGroup } from '../../types'
...
export function BenefitsUtilizationReportPage() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const groupSnap = await getDocs(query(collection(db, 'groups'), where('companyId', '==', currentCompanyId)))
      ...
      for (const payroll of payrolls) {
        const empBenefitSnap = await getDocs(
          query(collection(db, 'payroll_employee_benefits'), where('payrollId', '==', payroll.id))
        )
        const empBenefits = empBenefitSnap.docs.map(d => ({ id: d.id, ...d.data() })) as PayrollEmployeeBenefit[]
        ...
        for (const entry of empBenefits) {
          ...
          const group = payrollEmp?.groupId ? groupMap.get(payrollEmp.groupId) : null
  ...
  const totalEE = useMemo(() => benefits.reduce((sum, b) => sum + b.totalEmployeeShare, 0), [benefits])
  const totalER = useMemo(() => benefits.reduce((sum, b) => sum + b.totalEmployerShare, 0), [benefits])
  const totalCost = useMemo(() => benefits.reduce((sum, b) => sum + b.totalCost, 0), [benefits])
  const totalEmployees = useMemo(() => benefits.reduce((sum, b) => sum + b.employeeCount, 0), [benefits])
  ...
  const handleExportCSV = () => {
    const headers = ['Benefit', 'Employees Covered', 'EE Share', 'ER Share', 'Total Cost']
    ...
    rows.push(['TOTAL', totalEmployees.toString(), totalEE.toFixed(2), totalER.toFixed(2), totalCost.toFixed(2)])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Benefits Utilization Report</h1>
      ...
      {hasGenerated && (
        <>
          {benefits.length === 0 ? (
            <Card><CardContent className="pt-6"><p className="text-center text-gray-500 py-8">No benefit data found for the selected filters.</p></CardContent></Card>
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card>
                  <CardContent className="pt-6">
                    ...
                    <div className="text-2xl font-bold">{formatCurrency(totalEE)}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-6">
                    <div className="text-sm text-gray-500">Total ER Share</div>
                    <div className="text-2xl font-bold">{formatCurrency(totalER)}</div>
                  </CardContent>
                </Card>
              </div>
                ...
                <CardContent className="p-0">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Benefit</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {benefits.map((b) => (
                        <>
                          ...
                          >
                            <td className="px-6 py-4 text-sm font-medium text-gray-900">{b.benefitName}</td>
                            <td className="px-6 py-4 text-sm text-right">{b.employeeCount}</td>
                            <td className="px-6 py-4 text-sm text-right">{formatCurrency(b.totalEmployeeShare)}</td>
                            <td className="px-6 py-4 text-sm text-right">{formatCurrency(b.totalEmployerShare)}</td>
                            <td className="px-6 py-4 text-sm text-right font-semibold">{formatCurrency(b.totalCost)}</td>
                          </tr>
                                  ...
                                  <table className="w-full">
                                    <thead>
                                      <tr className="text-xs text-gray-500 border-b">
                                        ...
                                        <th className="text-left pb-2">Payroll</th>
                                        <th className="text-left pb-2">Period</th>
                                        <th className="text-right pb-2">EE Share</th>
                                        <th className="text-right pb-2">ER Share</th>
                                      </tr>
                                    </thead>
                                    <tbody>
                                      {b.employees.map((e, i) => (
                                        <tr key={i} className="border-b border-gray-100">
                                          <td className="py-2 text-sm text-gray-700">{e.employeeName}</td>
                                          <td className="py-2 text-sm text-gray-500">{e.groupName}</td>
                                          <td className="py-2 text-sm text-gray-500">{e.payrollName}</td>
                                          <td className="py-2 text-sm text-gray-500">{e.period}</td>
                                          <td className="py-2 text-sm text-right">{formatCurrency(e.employeeShare)}</td>
                                          <td className="py-2 text-sm text-right">{formatCurrency(e.employerShare)}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
                      ...
                      <tr className="bg-gray-50 font-bold border-t-2 border-gray-300">
                        ...
                        <td className="px-6 py-4 text-right">{formatCurrency(totalEE)}</td>
                        <td className="px-6 py-4 text-right">{formatCurrency(totalER)}</td>
