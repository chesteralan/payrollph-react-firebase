...
import type { Payroll, PayrollEmployeeBenefit, PayrollEmployeeEarning, PayrollEmployee } from '../../types'
...
export function YearEndReportPage() {
  ...
  const generateReport = async () => {
    ...
    try {
      const payrollSnap = await getDocs(
        query(collection(db, 'payroll'), where('companyId', '==', currentCompanyId), where('year', '==', selectedYear))
      )
      const payrolls = payrollSnap.docs.map(d => ({ id: d.id, ...d.data() })) as Payroll[]
      ...
      for (const payroll of payrolls) {
        const empSnap = await getDocs(
          query(collection(db, 'payroll_employees'), where('payrollId', '==', payroll.id))
        )
        const payrollEmployees = empSnap.docs.map(d => ({ id: d.id, ...d.data() })) as PayrollEmployee[]
        ...
        for (const emp of payrollEmployees) {
          ...
          const earningSnap = await getDocs(
            query(collection(db, 'payroll_employee_earnings'), where('payrollId', '==', payroll.id), where('nameId', '==', emp.nameId))
          )
          ...
          const benefitSnap = await getDocs(
            query(collection(db, 'payroll_employee_benefits'), where('payrollId', '==', payroll.id), where('nameId', '==', emp.nameId))
          )
          benefitSnap.docs.forEach(d => {
            const data = d.data() as PayrollEmployeeBenefit
            summary.totalBenefits += (data.employeeShare || 0) + (data.employerShare || 0)
          })
        }
      }
  ...
  const totals: YearEndTotals = useMemo(() => ({
    ...
    totalBasicSalary: summaries.reduce((sum, s) => sum + s.totalBasicSalary, 0),
    totalEarnings: summaries.reduce((sum, s) => sum + s.totalEarnings, 0),
    totalBenefits: summaries.reduce((sum, s) => sum + s.totalBenefits, 0),
    totalDeductions: summaries.reduce((sum, s) => sum + (s.totalGrossPay - s.totalNetPay - s.totalEarnings - s.totalBenefits), 0),
    total13thMonth: summaries.reduce((sum, s) => sum + s.total13thMonth, 0),
    totalEmployees: summaries.length,
    totalPayrollRuns: summaries.reduce((sum, s) => Math.max(sum, s.payrollRuns), 0),
  }), [summaries])
  ...
  const handleExportCSV = () => {
    const headers = ['Employee', 'Payroll Runs', 'Basic Salary', 'Total Earnings', 'Total Benefits', 'Gross Pay', 'Net Pay']
  ...
  return (
    <div className="space-y-6">
      ...
      {hasGenerated && (
        <>
          {summaries.length === 0 ? (
            <Card><CardContent className="pt-6"><p className="text-center text-gray-500 py-8">No payroll data found for {selectedYear}.</p></CardContent></Card>
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card>
                  <CardContent className="pt-6">
                    ...
                    <div className="text-2xl font-bold">{totals.totalPayrollRuns}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-6">
                    <div className="text-sm text-gray-500">Total Gross Pay</div>
                    <div className="text-2xl font-bold">{formatCurrency(totals.totalGrossPay)}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-6">
                    <div className="text-sm text-gray-500">Total Net Pay</div>
                    <div className="text-2xl font-bold">{formatCurrency(totals.totalNetPay)}</div>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card>
                  <CardContent className="pt-6">
                    ...
                    <div className="text-xl font-semibold">{formatCurrency(totals.totalBasicSalary)}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-6">
                    <div className="text-sm text-gray-500">Total Earnings</div>
                    <div className="text-xl font-semibold text-green-600">{formatCurrency(totals.totalEarnings)}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-6">
                    <div className="text-sm text-gray-500">Total Benefits (EE + ER)</div>
                    <div className="text-xl font-semibold text-blue-600">{formatCurrency(totals.totalBenefits)}</div>
                  </CardContent>
                </Card>
              </div>

              <Card>
                <CardHeader><CardTitle>Employee Year-End Summary</CardTitle></CardHeader>
                <CardContent className="p-0">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Employee</th>
                        <th className="text-right px-6 py-3 text-xs font-medium text-gray-500 uppercase">Runs</th>
                        <th className="text-right px-6 py-3 text-xs font-medium text-gray-500 uppercase">Basic Salary</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {summaries.map((s, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="px-6 py-4 text-sm font-medium text-gray-900">{s.employeeName}</td>
                          <td className="px-6 py-4 text-sm text-right">{s.payrollRuns}</td>
                          <td className="px-6 py-4 text-sm text-right">{formatCurrency(s.totalBasicSalary)}</td>
                          <td className="px-6 py-4 text-sm text-right">{formatCurrency(s.totalEarnings)}</td>
                          <td className="px-6 py-4 text-sm text-right">{formatCurrency(s.totalBenefits)}</td>
                          <td className="px-6 py-4 text-sm text-right font-medium">{formatCurrency(s.totalGrossPay)}</td>
                          <td className="px-6 py-4 text-sm text-right font-semibold">{formatCurrency(s.totalNetPay)}</td>
                        </tr>
                      ))}
                      <tr className="bg-gray-50 font-bold border-t-2 border-gray-300">
                        <td className="px-6 py-4">Total</td>
                        <td className="px-6 py-4 text-right">{totals.totalPayrollRuns}</td>
                        <td className="px-6 py-4 text-right">{formatCurrency(totals.totalBasicSalary)}</td>
                        <td className="px-6 py-4 text-right">{formatCurrency(totals.totalEarnings)}</td>
                        <td className="px-6 py-4 text-right">{formatCurrency(totals.totalBenefits)}</td>
                        <td className="px-6 py-4 text-right">{formatCurrency(totals.totalGrossPay)}</td>
                        <td className="px-6 py-4 text-right">{formatCurrency(totals.totalNetPay)}</td>
                      </tr>
                    </tbody>
                  </table>
                </CardContent>
