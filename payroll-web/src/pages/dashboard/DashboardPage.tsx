...
interface DashboardStats {
  ...
  recentPayrolls: { name: string; month: number; year: number; status: string; id: string; updatedAt: Date }[]
}
...
export function DashboardPage() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      const [employeesSnap, payrollsSnap, companiesSnap] = await Promise.all([
        ...
        getDocs(query(collection(db, 'payroll'), where('companyId', '==', currentCompanyId), orderBy('createdAt', 'desc'), limit(10))),
        getDocs(collection(db, 'companies'))
      ])

      const employees = employeesSnap.docs.map(d => ({ id: d.id, ...d.data() })) as { id: string; isActive?: boolean }[]
      const payrolls = payrollsSnap.docs.map(d => ({ id: d.id, ...d.data() })) as { id: string; name: string; month: number; year: number; status?: string; updatedAt?: Date }[]
      const companies = companiesSnap.docs.map(d => ({ id: d.id, ...d.data() }))

      const activeEmployees = employees.filter(e => e.isActive).length
      const publishedPayrolls = payrolls.filter(p => p.status === 'published').length
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card><CardContent className="p-0"><TableSkeleton rows={5} columns={3} /></CardContent></Card>
          <Card><CardContent className="p-0"><TableSkeleton rows={5} columns={3} /></CardContent></Card>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card) => (
          <Card key={card.label} className={card.isAction ? 'cursor-pointer hover:shadow-md transition-shadow' : ''} onClick={card.action}>
          ...
          <CardContent className="p-0">
            {stats.recentPayrolls.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <Button variant="ghost" className="mt-2" onClick={() => navigate('/payroll/new')}>
                ...
                {stats.recentPayrolls.map(payroll => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{getMonthName(payroll.month)} {payroll.year}</p>
                    </div>
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${statusColors[payroll.status]}`}>
                      {payroll.status}
                    </span>
                  </button>
                ))}
