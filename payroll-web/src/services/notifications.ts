...
export const createApprovalWorkflow = async (
  data: Omit<
    ApprovalWorkflow,
    "id" | "createdAt" | "status" | "currentLevel" | "approvals"
  >,
): Promise<string> => {
...
export const submitApproval = async (
  ...
  const isComplete =
    decision === "approved" && workflow.currentLevel + 1 >= workflow.maxLevel;
  const isRejected = decision === "rejected";

  await updateDoc(workflowRef, {
    approvals: updatedApprovals,
    currentLevel: isRejected
...
export const DEFAULT_APPROVAL_WORKFLOWS = {
  ...
  leave: {
    maxLevel: 1,
    levels: [{ level: 0, role: "supervisor" }],
  },
};
