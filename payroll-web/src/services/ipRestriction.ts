...
export const fetchIpRestrictions = async (
  companyId: string,
...
export const removeIpRestriction = async (
...
export const isIpAllowed = async (
  ...
  try {
    ...
    const isBlacklisted = blacklist.some((r) =>
      matchesIp(ipAddress, r.ipAddress, r.subnet),
    );
    if (isBlacklisted) {
    ...
    if (whitelist.length > 0) {
      const isWhitelisted = whitelist.some((r) =>
        matchesIp(ipAddress, r.ipAddress, r.subnet),
      );
      if (!isWhitelisted) {
...
export const parseCidr = (
  cidr: string,
): { baseIp: string; subnet: number } | null => {
