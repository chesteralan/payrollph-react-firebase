...
export const syncQueuedActions = async (): Promise<{ success: number; failed: number }> => {
  if (!navigator.onLine) {
      ...
      for (const action of actions) {
        try {
          ...
          if (newAttempts >= action.maxAttempts) {
            await updateQueuedActionStatus(action.id, 'failed', (error as Error).message)
          } else {
            await updateQueuedActionStatus(action.id, 'pending', (error as Error).message, newAttempts)
          }
          failed++
        }
      }
