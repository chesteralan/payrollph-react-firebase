...
export async function getPayrollWorkDays(
  ...
  const calendarEntries = calendarSnap.docs.map(d => ({ id: d.id, ...d.data() })) as (CalendarEntry & { isPaid?: boolean })[]
    ...
    if (calendarEntry) {
      if (calendarEntry.type === 'workday') {
        if (!isWeekday) {
          ...
          details.push({ date: dateStr, type: 'workday', name: calendarEntry.name })
        } else {
          totalWorkingDays++
          details.push({ date: dateStr, type: 'working', name: calendarEntry.name })
        }
      } else if (calendarEntry.type === 'holiday' || calendarEntry.type === 'special') {
        if (isWeekday) {
          holidaysSubtracted++
          details.push({ date: dateStr, type: calendarEntry.type, name: calendarEntry.name })
        } else {
          details.push({ date: dateStr, type: 'weekend', name: calendarEntry.name })
        }
      }
    } else if (isWeekday) {
      totalWorkingDays++
  ...
  return { totalWorkingDays, holidaysSubtracted, specialWorkdaysAdded, details }
}
