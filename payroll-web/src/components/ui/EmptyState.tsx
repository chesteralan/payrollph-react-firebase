...
import { FileText, Users, DollarSign, Calendar, BarChart3, Settings, FolderOpen, AlertCircle } from 'lucide-react'

interface EmptyStateProps {
  icon?: ReactNode
  type?: 'data' | 'employees' | 'payroll' | 'calendar' | 'reports' | 'settings' | 'files' | 'error'
  title: string
  description?: string
  action?: ReactNode
  className?: string
...
export function EmptyState({ icon, type, title, description, action, className = '' }: EmptyStateProps) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center py-16 px-4 text-center ${className}`} role="status">
      <div className="mb-4" aria-hidden="true">{displayIcon}</div>
